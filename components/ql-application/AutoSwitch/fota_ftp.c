...
//FTP服务器相关信息
char hostname[] = "139.196.252.149:21\0";         //FTP服务器 IP:端口号
char username[] = "test\0";                         //FTP服务器用户名
char password[] = "test\0";                         //FTP服务器用户密码
char file_full_path[50] = "ASW_EC200U/";    //FTP服务器文件路径
...
int fota_ftp_client_init(fota_ftp_client_t *fota_ftp_cli_p)
{
	...
	//FTP服务器相关信息
    //char FileName[50];
    memset(file_full_path,'\0',strlen(file_full_path));
    sprintf(file_full_path,(char *)"ASW_EC200U/Fota_%s.pack",(char *) SOFT_VER);
   // strcat(file_full_path,FileName);
    ...
    QL_FOTA_FTP_LOG("FOTA FULL PATH %s",file_full_path);
...
int fota_ftp_net_register(uint8_t nSim,int profile_idx)
{
    ...
    for (int i = 0; i < 10; i++)
    {
        ret = ql_network_register_wait(nSim, 30);
    ...
    if(ql_datacall_get_sim_profile_is_active(nSim,profile_idx)==0)
    {
    ...
    ret = ql_datacall_get_default_pdn_info(nSim,apn_info);  
    QL_FOTA_FTP_LOG("NW: ret=0x%x, ip_version:%d apn_name:%s", 
                ret, apn_info->ip_version,apn_info->apn_name);  
    ret=ql_start_data_call(nSim, profile_idx , apn_info->ip_version,apn_info->apn_name, NULL, NULL, 0);
    QL_FOTA_FTP_LOG("NW:===data call result:%d", ret);
    if(ret != 0)
    {
    QL_FOTA_FTP_LOG("NW:====data call failure!!!!=====");
    }                 
...
void fota_ftp_file_check(fota_ftp_client_t *fota_ftp_cli_p)
{
    ...
    if (DOWN_DOWNED == fota_ftp_cli_p->down_state)
    {
        ...
        else
        {
            ...
            strcpy(SendSmsBuff,"UPDATE SUCCESSFULLY..!");
            ReplyToUser(SmsRevMob,SendSmsBuff,(int *)SMS_MODE);
...
init_error_exit:
            ...
            SendSmsBuff = malloc(150);
            memset(SendSmsBuff,'\0',150);
            sprintf(SendSmsBuff,"UPDATE FAIL %d \r\n",FotaFail);
            ReplyToUser(SmsRevMob,SendSmsBuff,(int *)SMS_MODE);
