...
static void ql_rtc_demo_thread()
{
   ...
   ql_rtc_cfg_t rtc_config;
   rtc_config.nv_cfg = QUEC_DISABLE;
   rtc_config.nwt_cfg = QUEC_ENABLE;
   rtc_config.rtc_cfg = QUEC_DISABLE;
   rtc_config.tz_cfg  = 0;
    ql_rtc_set_cfg(&rtc_config);
    ...
    tm.tm_min += 1;
	  ret = ql_rtc_set_alarm(&tm);
     QL_RTCDEMO_LOG("=========ql_rtc_set_alarm= %d ===========\r\n",ret);
     QL_RTCDEMO_LOG("=========print alarm time===========\r\n");
      ql_rtc_print_time(tm);


    ret = ql_rtc_register_cb(ql_rtc_test_callback);
     QL_RTCDEMO_LOG("=========ql_rtc_register_cb= %d ===========\r\n",ret);
     
      ret = ql_rtc_enable_alarm(1);
        QL_RTCDEMO_LOG("=========ql_rtc_enable_alarm= %d ===========\r\n",ret);
